...
public class BusLineList
{
   ...
   transient HashMap<Integer, BusLine> cache = null;

   public BusLine findBusLine(int LI_NR)
   {
      if (cache == null)
      {
         cache = new HashMap<Integer, BusLine>();
         for (BusLine busLine: list)
         {
            cache.put(busLine.getLI_NR(), busLine);
         }
      }
      return cache.get(LI_NR);
   }

   public BusLine[] getList()
   {
      return list;
   }
}
